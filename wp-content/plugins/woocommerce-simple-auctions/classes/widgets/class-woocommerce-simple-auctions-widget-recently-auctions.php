...
class WC_SA_Widget_Recently_Viewed_Auction extends WP_Widget {
     ...
     * 
	 */
	function WC_SA_Widget_Recently_Viewed_Auction() {
		...
		/* Create the widget. */
		$this->WP_Widget('recently_viewed_auctions', $this->woo_widget_name, $widget_ops);
		...
		if ( $r->have_posts() ) {
			...
			while ( $r->have_posts()) {
				...
				if($hide_time != 1 && !$product->is_closed())
					...
					<div class="auction-time-countdown" data-time="'.$product->get_seconds_remaining().'" data-auctionid="'.$product->id.'" data-format="'.get_option( 'simple_auctions_countdown_format' ).'"></div>';
